class Digrafo:
    ...
    def add_aresta(self,v,u,p):
        self.add_vertice(v)
        self.add_vertice(u)
        self.lista_de_adjacencia[u].append((v,p))
    ...
    def grau_do_vertice(self,v):
        grau_saida = len(self.lista_de_adjacencia[v])
        grau_entrada = 0
        for i in range(self.num_vertices()):
            for k in self.lista_de_adjacencia[i+1]:
                if k[0] == v:
                    grau_entrada += 1
    ...
    def min_grau(self):
        ...
        for k in list(self.lista_de_adjacencia.keys()):
            l.append(self.grau_do_vertice(k))
        return min(l)
    ...
    def bfs(self,v):
        ...
        while len(visitados) != len(self.lista_de_adjacencia):
            d+=1
            for n in self.lista_de_adjacencia[fila[0]]:
                if n[0] not in visitados:
                    visitados.add(n[0])
                    fila.append(n[0])
                    pais.append((n[0],fila[0]))
                    distancia.append((n[0],d))
            fila.pop(0)
    ...
    def dijkstra(self, vertice):
        dist = [float('inf')] * self.numeroDeVertices()  #inicializa a lista de distâncias como infinito para todos os vértices
        dist[vertice - 1] = 0  #define a distância do vértice inicial como 0
        pais = [None] * self.numeroDeVertices()  #inicializa a lista de pais para reconstruir o caminho mínimo, com valor None
